...
public class JetpackCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!cmd.getName().equalsIgnoreCase("simplejetpacks")) {
            SimpleJetpacks.getPlugin().getLogger().warning("Non-simplejetpacks command sent to SimpleJetpacks, this is a bug");
            return true; // we shouldn't be the one to handle this...
        }
        if (args.length == 0) {
